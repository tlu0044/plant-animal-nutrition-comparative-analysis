...
# 1. Descriptive Statistics (Includes B12)
...
# 2. Nutrient Density (Nutrients per 100kcal, Includes B12)
density_df = df.copy()
...
# 3. Updated Powerhouse Rankings (Top 5 per Category, Includes B12)
ranking_list = []
...
# 3. Macro Calorie Ratios (% of Total Energy)
# Shows the average percentage of energy derived from each macronutrient
ratio_df = df.copy()
...
# 4. B12 Presence Rate
# Percentage of foods in each category with B12 > 0
...
# 5. Correlation Heatmap (Includes B12)
plt.figure(figsize=(10, 8))
corr = df[['Protein', 'Fat', 'Carbs', 'Fiber', 'Calories', 'Calcium', 'Iron', 'Vitamin B12']].corr()
sns.heatmap(corr, annot=True, cmap='coolwarm', fmt=".2f", linewidths=.5)
plt.title('Nutrient Correlation Heatmap (Including B12)')
plt.tight_layout()
plt.savefig(output_path/'correlation_heatmap.png')
plt.close()

print("All statistical summaries including B12 have been generated.")